...
[Scenario(
    specId: "VCHIP-8016-US01-SC04",
    title: "Change page size above server max clamps to 500",
    given: "Given more than 500 Devices exist",
    when:  "When I GET /api/devices?skip=0&take=2000",
...
public sealed class SC04_ChangePageSize_ClampsTo500 : WhenTestingForAsync<AppFactory<Program>>
{
    ...
    {
        var items = await _client.GetFromJsonAsync<List<Device>>("/api/devices?skip=0&take=2000");
