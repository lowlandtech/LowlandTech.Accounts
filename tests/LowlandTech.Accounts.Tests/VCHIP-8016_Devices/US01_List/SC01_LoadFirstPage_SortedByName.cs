...
[Scenario(
    specId: "VCHIP-8016-US01-SC01",
    title: "Load first page sorted by Name",
    given: "Given the API has Devices seeded",
    when:  "When I GET /api/devices?skip=0&take=100",
...
public sealed class SC01_LoadFirstPage_SortedByName : WhenTestingForAsync<AppFactory<Program>>
{
    ...
    {
        var res = await _client.GetAsync("/api/devices?skip=0&take=100");
    ...
    {
        var items = await _client.GetFromJsonAsync<List<Device>>("/api/devices?skip=0&take=100");
    ...
    [Fact]
    [Then("The table renders the returned Devices", "UAC003")]
