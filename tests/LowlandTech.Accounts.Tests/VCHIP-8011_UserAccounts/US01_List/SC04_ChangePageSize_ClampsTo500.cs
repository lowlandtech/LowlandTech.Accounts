...
[Scenario(
    specId: "VCHIP-8011-US01-SC04",
    title: "Change page size above server max clamps to 500",
    given: "Given more than 500 UserAccounts exist",
    when:  "When I GET /api/useraccounts?skip=0&take=2000",
...
public sealed class SC04_ChangePageSize_ClampsTo500 : WhenTestingForAsync<AppFactory<Program>>
{
    ...
    [Fact]
    [Then("It calls GET /api/useraccounts?skip=0&take=500", "UAC008")]
    public async Task Server_Clamps_Take_To_500()
    {
        // Client asks for 2000; server clamps to 500 which we validate via count
        var res = await _client.GetAsync("/api/useraccounts?skip=0&take=2000");
    ...
    {
        var items = await _client.GetFromJsonAsync<List<UserAccount>>("/api/useraccounts?skip=0&take=2000");
