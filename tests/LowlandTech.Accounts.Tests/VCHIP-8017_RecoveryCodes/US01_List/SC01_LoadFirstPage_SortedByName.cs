...
[Scenario(
    specId: "VCHIP-8017-US01-SC01",
    title: "Load first page sorted by Name",
    given: "Given the API has RecoveryCodes seeded",
    when:  "When I GET /api/recoverycodes?skip=0&take=100",
...
public sealed class SC01_LoadFirstPage_SortedByName : WhenTestingForAsync<AppFactory<Program>>
{
    ...
    {
        var res = await _client.GetAsync("/api/recoverycodes?skip=0&take=100");
    ...
    {
        var items = await _client.GetFromJsonAsync<List<RecoveryCode>>("/api/recoverycodes?skip=0&take=100");
    ...
    [Fact]
    [Then("The table renders the returned RecoveryCodes", "UAC003")]
