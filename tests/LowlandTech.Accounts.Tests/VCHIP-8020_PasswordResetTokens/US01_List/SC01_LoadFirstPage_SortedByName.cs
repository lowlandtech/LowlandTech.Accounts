...
[Scenario(
    specId: "VCHIP-8020-US01-SC01",
    title: "Load first page sorted by Name",
    given: "Given the API has PasswordResetTokens seeded",
    when:  "When I GET /api/passwordresettokens?skip=0&take=100",
...
public sealed class SC01_LoadFirstPage_SortedByName : WhenTestingForAsync<AppFactory<Program>>
{
    ...
    {
        var res = await _client.GetAsync("/api/passwordresettokens?skip=0&take=100");
    ...
    {
        var items = await _client.GetFromJsonAsync<List<PasswordResetToken>>("/api/passwordresettokens?skip=0&take=100");
    ...
    [Fact]
    [Then("The table renders the returned PasswordResetTokens", "UAC003")]
