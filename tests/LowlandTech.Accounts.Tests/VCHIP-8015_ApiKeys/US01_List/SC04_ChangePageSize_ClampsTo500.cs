...
[Scenario(
    specId: "VCHIP-8015-US01-SC04",
    title: "Change page size above server max clamps to 500",
    given: "Given more than 500 ApiKeys exist",
    when:  "When I GET /api/apikeys?skip=0&take=2000",
...
public sealed class SC04_ChangePageSize_ClampsTo500 : WhenTestingForAsync<AppFactory<Program>>
{
    ...
    {
        var items = await _client.GetFromJsonAsync<List<ApiKey>>("/api/apikeys?skip=0&take=2000");
