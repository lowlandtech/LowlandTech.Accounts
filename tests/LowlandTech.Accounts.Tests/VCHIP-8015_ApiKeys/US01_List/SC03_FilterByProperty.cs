...
[Scenario(
    specId: "VCHIP-8015-US01-SC03",
    title: "Filter by property",
    given: "Given the API has ApiKeys with filterable properties",
    when:  "When I GET /api/apikeys?skip=0&take=100&filterProperty=filterValue",
    then:  "Then only ApiKeys matching the filter are returned")]
public sealed class SC03_FilterByProperty : WhenTestingForAsync<AppFactory<Program>>
{
    ...
    [Fact]
    [Then("It calls GET /api/apikeys with the filter parameter", "UAC006")]
    public async Task Calls_Endpoint_With_Filter()
    {
        // TODO: Customize filter property and value for your table
        var res = await _client.GetAsync("/api/apikeys?skip=0&take=100&filterProperty=filterValue");
        res.IsSuccessStatusCode.ShouldBeTrue();
    }

    [Fact]
    [Then("Only matching ApiKeys are shown", "UAC007")]
    public async Task Returns_Only_Matching_Items()
    {
        // TODO: Customize filter property and value for your table
        var items = await _client.GetFromJsonAsync<List<ApiKey>>("/api/apikeys?skip=0&take=100&filterProperty=filterValue");
