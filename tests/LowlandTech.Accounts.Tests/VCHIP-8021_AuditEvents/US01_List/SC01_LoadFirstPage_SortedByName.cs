...
[Scenario(
    specId: "VCHIP-8021-US01-SC01",
    title: "Load first page sorted by Name",
    given: "Given the API has AuditEvents seeded",
    when:  "When I GET /api/auditevents?skip=0&take=100",
...
public sealed class SC01_LoadFirstPage_SortedByName : WhenTestingForAsync<AppFactory<Program>>
{
    ...
    {
        var res = await _client.GetAsync("/api/auditevents?skip=0&take=100");
    ...
    {
        var items = await _client.GetFromJsonAsync<List<AuditEvent>>("/api/auditevents?skip=0&take=100");
    ...
    [Fact]
    [Then("The table renders the returned AuditEvents", "UAC003")]
