...
[Scenario(
    specId: "VCHIP-8021-US01-SC02",
    title: "Search by Id or Name",
    given: "Given the API has AuditEvents with searchable Ids and Names",
    when:  "When I GET /api/auditevents?skip=0&take=100&search={term}",
    then:  "Then only AuditEvents whose Id or Name contains the search term are returned")]
public sealed class SC02_SearchByIdOrName : WhenTestingForAsync<AppFactory<Program>>
{
    ...
    [Fact]
    [Then("It calls GET /api/auditevents?skip=0&take=100&search={term}", "UAC004")]
    public async Task Calls_Endpoint_With_Search_Query()
    {
        var res = await _client.GetAsync("/api/auditevents?skip=0&take=100&search=test");
        res.IsSuccessStatusCode.ShouldBeTrue();
    }

    [Fact]
    [Then("Only AuditEvents whose Id or Name contains the search term are shown", "UAC005")]
    public async Task Filters_By_Id_Or_Name_Contains()
    {
        var items = await _client.GetFromJsonAsync<List<AuditEvent>>("/api/auditevents?skip=0&take=100&search=test");
