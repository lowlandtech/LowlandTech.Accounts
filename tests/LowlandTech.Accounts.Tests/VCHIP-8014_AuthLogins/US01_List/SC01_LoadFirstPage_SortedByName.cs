...
[Scenario(
    specId: "VCHIP-8014-US01-SC01",
    title: "Load first page sorted by Name",
    given: "Given the API has AuthLogins seeded",
    when:  "When I GET /api/authlogins?skip=0&take=100",
...
public sealed class SC01_LoadFirstPage_SortedByName : WhenTestingForAsync<AppFactory<Program>>
{
    ...
    {
        var res = await _client.GetAsync("/api/authlogins?skip=0&take=100");
    ...
    {
        var items = await _client.GetFromJsonAsync<List<AuthLogin>>("/api/authlogins?skip=0&take=100");
    ...
    [Fact]
    [Then("The table renders the returned AuthLogins", "UAC003")]
