...
public sealed class RecoveryCodePageState : INotificationHandler<RecoveryCodeInitEvent>
{
    public event Action? Changed;
    private readonly StateBag _bag = new();
    private readonly IMediator _mediator;

    private readonly StateSlice<IReadOnlyList<RecoveryCodeDto>> _itemsSlice;
    ...
    public RecoveryCodePageState(IMediator mediator)
    {
        _mediator = mediator;
        _itemsSlice    = _bag.GetSlice<IReadOnlyList<RecoveryCodeDto>>("Items");
    ...
    public RecoveryCodeDto? Selected { get; private set; }
    public RecoveryCodeDto  Editing  { get; private set; } = new();

    public IReadOnlyList<RecoveryCodeDto> Items
    {
    ...
    public int Page
    {
        ...
        set { _ = _mediator.Send(new RecoveryCodeSetPageAction(value)); }
    }

    public int PageSize
    {
        get => _pageSizeSlice.Value;
        set { _ = _mediator.Send(new RecoveryCodeSetPageSizeAction(value)); }
    }

    public string? Query
    {
        get => _querySlice.Value;
        set { _ = _mediator.Send(new RecoveryCodeSetQueryAction(value)); }
    }

    public string? SortBy
    {
        get => _sortBySlice.Value;
        set { _ = _mediator.Send(new RecoveryCodeSetSortAction(value, SortDir)); }
    }

    public string? SortDir
    {
        get => _sortDirSlice.Value;
        set { _ = _mediator.Send(new RecoveryCodeSetSortAction(SortBy, value)); }
    }
    ...
    public Task Load(int? page = null, int? pageSize = null, string? q = null, string? sort = null, string? dir = null)
        => _mediator.Send(new RecoveryCodeInitAction(page ?? Page, pageSize ?? PageSize, q ?? Query, sort ?? SortBy, dir ?? SortDir));

    public void Select(RecoveryCodeDto? item)
    {
        Selected = item;
        Editing  = item is null
            ? new()
            : JsonSerializer.Deserialize<RecoveryCodeDto>(
    ...
    public Task Handle(RecoveryCodeInitEvent e, CancellationToken ct)
    {
        ReducePage(e.Page);
