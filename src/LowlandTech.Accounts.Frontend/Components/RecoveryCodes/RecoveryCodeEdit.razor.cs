...
public partial class RecoveryCodeEdit : ComponentBase, IDisposable
{
    [Inject] private AppState App { get; set; } = default!;
    [Inject] private IPermService Perms { get; set; } = default!;
    [Inject] private RecoveryCodePageState State { get; set; } = default!;
    ...
    {
        State.ItemsChanged += OnRecoveryCodeChanged;
        _canCreate = await Perms.CanAsync("recoverycode::create");
        _canUpdate = await Perms.CanAsync("recoverycode::update");
        _canDelete = await Perms.CanAsync("recoverycode::delete");
    }

    private void OnRecoveryCodeChanged() => InvokeAsync(StateHasChanged);

    public void Dispose() => State.ItemsChanged -= OnRecoveryCodeChanged;
