...
public sealed class AccountPreferencePageState : INotificationHandler<AccountPreferenceInitEvent>
{
    public event Action? Changed;
    private readonly StateBag _bag = new();
    private readonly IMediator _mediator;

    private readonly StateSlice<IReadOnlyList<AccountPreferenceDto>> _itemsSlice;
    ...
    public AccountPreferencePageState(IMediator mediator)
    {
        _mediator = mediator;
        _itemsSlice    = _bag.GetSlice<IReadOnlyList<AccountPreferenceDto>>("Items");
    ...
    public AccountPreferenceDto? Selected { get; private set; }
    public AccountPreferenceDto  Editing  { get; private set; } = new();

    public IReadOnlyList<AccountPreferenceDto> Items
    {
    ...
    public int Page
    {
        ...
        set { _ = _mediator.Send(new AccountPreferenceSetPageAction(value)); }
    }

    public int PageSize
    {
        get => _pageSizeSlice.Value;
        set { _ = _mediator.Send(new AccountPreferenceSetPageSizeAction(value)); }
    }

    public string? Query
    {
        get => _querySlice.Value;
        set { _ = _mediator.Send(new AccountPreferenceSetQueryAction(value)); }
    }

    public string? SortBy
    {
        get => _sortBySlice.Value;
        set { _ = _mediator.Send(new AccountPreferenceSetSortAction(value, SortDir)); }
    }

    public string? SortDir
    {
        get => _sortDirSlice.Value;
        set { _ = _mediator.Send(new AccountPreferenceSetSortAction(SortBy, value)); }
    }
    ...
    public Task Load(int? page = null, int? pageSize = null, string? q = null, string? sort = null, string? dir = null)
        => _mediator.Send(new AccountPreferenceInitAction(page ?? Page, pageSize ?? PageSize, q ?? Query, sort ?? SortBy, dir ?? SortDir));

    public void Select(AccountPreferenceDto? item)
    {
        Selected = item;
        Editing  = item is null
            ? new()
            : JsonSerializer.Deserialize<AccountPreferenceDto>(
    ...
    public Task Handle(AccountPreferenceInitEvent e, CancellationToken ct)
    {
        ReducePage(e.Page);
