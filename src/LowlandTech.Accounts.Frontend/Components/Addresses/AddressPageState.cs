...
public sealed class AddressPageState : INotificationHandler<AddressInitEvent>
{
    public event Action? Changed;
    private readonly StateBag _bag = new();
    private readonly IMediator _mediator;

    private readonly StateSlice<IReadOnlyList<AddressDto>> _itemsSlice;
    ...
    public AddressPageState(IMediator mediator)
    {
        _mediator = mediator;
        _itemsSlice    = _bag.GetSlice<IReadOnlyList<AddressDto>>("Items");
    ...
    public AddressDto? Selected { get; private set; }
    public AddressDto  Editing  { get; private set; } = new();

    public IReadOnlyList<AddressDto> Items
    {
    ...
    public int Page
    {
        ...
        set { _ = _mediator.Send(new AddressSetPageAction(value)); }
    }

    public int PageSize
    {
        get => _pageSizeSlice.Value;
        set { _ = _mediator.Send(new AddressSetPageSizeAction(value)); }
    }

    public string? Query
    {
        get => _querySlice.Value;
        set { _ = _mediator.Send(new AddressSetQueryAction(value)); }
    }

    public string? SortBy
    {
        get => _sortBySlice.Value;
        set { _ = _mediator.Send(new AddressSetSortAction(value, SortDir)); }
    }

    public string? SortDir
    {
        get => _sortDirSlice.Value;
        set { _ = _mediator.Send(new AddressSetSortAction(SortBy, value)); }
    }
    ...
    public Task Load(int? page = null, int? pageSize = null, string? q = null, string? sort = null, string? dir = null)
        => _mediator.Send(new AddressInitAction(page ?? Page, pageSize ?? PageSize, q ?? Query, sort ?? SortBy, dir ?? SortDir));

    public void Select(AddressDto? item)
    {
        Selected = item;
        Editing  = item is null
            ? new()
            : JsonSerializer.Deserialize<AddressDto>(
    ...
    public Task Handle(AddressInitEvent e, CancellationToken ct)
    {
        ReducePage(e.Page);
