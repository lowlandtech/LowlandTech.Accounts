...
public partial class ApiKeyEdit : ComponentBase, IDisposable
{
    [Inject] private AppState App { get; set; } = default!;
    [Inject] private IPermService Perms { get; set; } = default!;
    [Inject] private ApiKeyPageState State { get; set; } = default!;
    ...
    {
        State.ItemsChanged += OnApiKeyChanged;
        _canCreate = await Perms.CanAsync("apikey::create");
        _canUpdate = await Perms.CanAsync("apikey::update");
        _canDelete = await Perms.CanAsync("apikey::delete");
    }

    private void OnApiKeyChanged() => InvokeAsync(StateHasChanged);

    public void Dispose() => State.ItemsChanged -= OnApiKeyChanged;
