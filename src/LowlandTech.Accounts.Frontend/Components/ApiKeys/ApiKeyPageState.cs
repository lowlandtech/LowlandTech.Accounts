...
public sealed class ApiKeyPageState : INotificationHandler<ApiKeyInitEvent>
{
    public event Action? Changed;
    private readonly StateBag _bag = new();
    private readonly IMediator _mediator;

    private readonly StateSlice<IReadOnlyList<ApiKeyDto>> _itemsSlice;
    ...
    public ApiKeyPageState(IMediator mediator)
    {
        _mediator = mediator;
        _itemsSlice    = _bag.GetSlice<IReadOnlyList<ApiKeyDto>>("Items");
    ...
    public ApiKeyDto? Selected { get; private set; }
    public ApiKeyDto  Editing  { get; private set; } = new();

    public IReadOnlyList<ApiKeyDto> Items
    {
    ...
    public int Page
    {
        ...
        set { _ = _mediator.Send(new ApiKeySetPageAction(value)); }
    }

    public int PageSize
    {
        get => _pageSizeSlice.Value;
        set { _ = _mediator.Send(new ApiKeySetPageSizeAction(value)); }
    }

    public string? Query
    {
        get => _querySlice.Value;
        set { _ = _mediator.Send(new ApiKeySetQueryAction(value)); }
    }

    public string? SortBy
    {
        get => _sortBySlice.Value;
        set { _ = _mediator.Send(new ApiKeySetSortAction(value, SortDir)); }
    }

    public string? SortDir
    {
        get => _sortDirSlice.Value;
        set { _ = _mediator.Send(new ApiKeySetSortAction(SortBy, value)); }
    }
    ...
    public Task Load(int? page = null, int? pageSize = null, string? q = null, string? sort = null, string? dir = null)
        => _mediator.Send(new ApiKeyInitAction(page ?? Page, pageSize ?? PageSize, q ?? Query, sort ?? SortBy, dir ?? SortDir));

    public void Select(ApiKeyDto? item)
    {
        Selected = item;
        Editing  = item is null
            ? new()
            : JsonSerializer.Deserialize<ApiKeyDto>(
    ...
    public Task Handle(ApiKeyInitEvent e, CancellationToken ct)
    {
        ReducePage(e.Page);
