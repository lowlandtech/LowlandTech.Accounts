...
public sealed class AuthLoginPageState : INotificationHandler<AuthLoginInitEvent>
{
    public event Action? Changed;
    private readonly StateBag _bag = new();
    private readonly IMediator _mediator;

    private readonly StateSlice<IReadOnlyList<AuthLoginDto>> _itemsSlice;
    ...
    public AuthLoginPageState(IMediator mediator)
    {
        _mediator = mediator;
        _itemsSlice    = _bag.GetSlice<IReadOnlyList<AuthLoginDto>>("Items");
    ...
    public AuthLoginDto? Selected { get; private set; }
    public AuthLoginDto  Editing  { get; private set; } = new();

    public IReadOnlyList<AuthLoginDto> Items
    {
    ...
    public int Page
    {
        ...
        set { _ = _mediator.Send(new AuthLoginSetPageAction(value)); }
    }

    public int PageSize
    {
        get => _pageSizeSlice.Value;
        set { _ = _mediator.Send(new AuthLoginSetPageSizeAction(value)); }
    }

    public string? Query
    {
        get => _querySlice.Value;
        set { _ = _mediator.Send(new AuthLoginSetQueryAction(value)); }
    }

    public string? SortBy
    {
        get => _sortBySlice.Value;
        set { _ = _mediator.Send(new AuthLoginSetSortAction(value, SortDir)); }
    }

    public string? SortDir
    {
        get => _sortDirSlice.Value;
        set { _ = _mediator.Send(new AuthLoginSetSortAction(SortBy, value)); }
    }
    ...
    public Task Load(int? page = null, int? pageSize = null, string? q = null, string? sort = null, string? dir = null)
        => _mediator.Send(new AuthLoginInitAction(page ?? Page, pageSize ?? PageSize, q ?? Query, sort ?? SortBy, dir ?? SortDir));

    public void Select(AuthLoginDto? item)
    {
        Selected = item;
        Editing  = item is null
            ? new()
            : JsonSerializer.Deserialize<AuthLoginDto>(
    ...
    public Task Handle(AuthLoginInitEvent e, CancellationToken ct)
    {
        ReducePage(e.Page);
