...
public partial class AuthLoginEdit : ComponentBase, IDisposable
{
    [Inject] private AppState App { get; set; } = default!;
    [Inject] private IPermService Perms { get; set; } = default!;
    [Inject] private AuthLoginPageState State { get; set; } = default!;
    ...
    {
        State.ItemsChanged += OnAuthLoginChanged;
        _canCreate = await Perms.CanAsync("authlogin::create");
        _canUpdate = await Perms.CanAsync("authlogin::update");
        _canDelete = await Perms.CanAsync("authlogin::delete");
    }

    private void OnAuthLoginChanged() => InvokeAsync(StateHasChanged);

    public void Dispose() => State.ItemsChanged -= OnAuthLoginChanged;
