...
public sealed class AccountPageState : INotificationHandler<AccountInitEvent>
{
    public event Action? Changed;
    private readonly StateBag _bag = new();
    private readonly IMediator _mediator;

    private readonly StateSlice<IReadOnlyList<AccountDto>> _itemsSlice;
    ...
    public AccountPageState(IMediator mediator)
    {
        _mediator = mediator;
        _itemsSlice    = _bag.GetSlice<IReadOnlyList<AccountDto>>("Items");
    ...
    public AccountDto? Selected { get; private set; }
    public AccountDto  Editing  { get; private set; } = new();

    public IReadOnlyList<AccountDto> Items
    {
    ...
    public int Page
    {
        ...
        set { _ = _mediator.Send(new AccountSetPageAction(value)); }
    }

    public int PageSize
    {
        get => _pageSizeSlice.Value;
        set { _ = _mediator.Send(new AccountSetPageSizeAction(value)); }
    }

    public string? Query
    {
        get => _querySlice.Value;
        set { _ = _mediator.Send(new AccountSetQueryAction(value)); }
    }

    public string? SortBy
    {
        get => _sortBySlice.Value;
        set { _ = _mediator.Send(new AccountSetSortAction(value, SortDir)); }
    }

    public string? SortDir
    {
        get => _sortDirSlice.Value;
        set { _ = _mediator.Send(new AccountSetSortAction(SortBy, value)); }
    }
    ...
    public Task Load(int? page = null, int? pageSize = null, string? q = null, string? sort = null, string? dir = null)
        => _mediator.Send(new AccountInitAction(page ?? Page, pageSize ?? PageSize, q ?? Query, sort ?? SortBy, dir ?? SortDir));

    public void Select(AccountDto? item)
    {
        Selected = item;
    ...
    public Task Handle(AccountInitEvent e, CancellationToken ct)
    {
        ReducePage(e.Page);
