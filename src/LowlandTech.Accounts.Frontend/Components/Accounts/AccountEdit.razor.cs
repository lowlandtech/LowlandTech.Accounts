...
public partial class AccountEdit : ComponentBase, IDisposable
{
    [Inject] private AppState App { get; set; } = default!;
    [Inject] private IPermService Perms { get; set; } = default!;
    [Inject] private AccountPageState State { get; set; } = default!;
    ...
    {
        State.ItemsChanged += OnAccountChanged;
        _canCreate = await Perms.CanAsync("account::create");
        _canUpdate = await Perms.CanAsync("account::update");
        _canDelete = await Perms.CanAsync("account::delete");
    }

    private void OnAccountChanged() => InvokeAsync(StateHasChanged);

    public void Dispose() => State.ItemsChanged -= OnAccountChanged;
