...
public partial class UserAccountEdit : ComponentBase, IDisposable
{
    [Inject] private AppState App { get; set; } = default!;
    [Inject] private IPermService Perms { get; set; } = default!;
    [Inject] private UserAccountPageState State { get; set; } = default!;
    ...
    {
        State.ItemsChanged += OnUserAccountChanged;
        _canCreate = await Perms.CanAsync("useraccount::create");
        _canUpdate = await Perms.CanAsync("useraccount::update");
        _canDelete = await Perms.CanAsync("useraccount::delete");
    }

    private void OnUserAccountChanged() => InvokeAsync(StateHasChanged);

    public void Dispose() => State.ItemsChanged -= OnUserAccountChanged;
