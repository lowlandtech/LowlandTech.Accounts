...
public partial class EmailVerificationTokenEdit : ComponentBase, IDisposable
{
    [Inject] private AppState App { get; set; } = default!;
    [Inject] private IPermService Perms { get; set; } = default!;
    [Inject] private EmailVerificationTokenPageState State { get; set; } = default!;
    ...
    {
        State.ItemsChanged += OnEmailVerificationTokenChanged;
        _canCreate = await Perms.CanAsync("emailverificationtoken::create");
        _canUpdate = await Perms.CanAsync("emailverificationtoken::update");
        _canDelete = await Perms.CanAsync("emailverificationtoken::delete");
    }

    private void OnEmailVerificationTokenChanged() => InvokeAsync(StateHasChanged);

    public void Dispose() => State.ItemsChanged -= OnEmailVerificationTokenChanged;
