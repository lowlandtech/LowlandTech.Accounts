...
public sealed class AuditEventPageState : INotificationHandler<AuditEventInitEvent>
{
    public event Action? Changed;
    private readonly StateBag _bag = new();
    private readonly IMediator _mediator;

    private readonly StateSlice<IReadOnlyList<AuditEventDto>> _itemsSlice;
    ...
    public AuditEventPageState(IMediator mediator)
    {
        _mediator = mediator;
        _itemsSlice    = _bag.GetSlice<IReadOnlyList<AuditEventDto>>("Items");
    ...
    public AuditEventDto? Selected { get; private set; }
    public AuditEventDto  Editing  { get; private set; } = new();

    public IReadOnlyList<AuditEventDto> Items
    {
    ...
    public int Page
    {
        ...
        set { _ = _mediator.Send(new AuditEventSetPageAction(value)); }
    }

    public int PageSize
    {
        get => _pageSizeSlice.Value;
        set { _ = _mediator.Send(new AuditEventSetPageSizeAction(value)); }
    }

    public string? Query
    {
        get => _querySlice.Value;
        set { _ = _mediator.Send(new AuditEventSetQueryAction(value)); }
    }

    public string? SortBy
    {
        get => _sortBySlice.Value;
        set { _ = _mediator.Send(new AuditEventSetSortAction(value, SortDir)); }
    }

    public string? SortDir
    {
        get => _sortDirSlice.Value;
        set { _ = _mediator.Send(new AuditEventSetSortAction(SortBy, value)); }
    }
    ...
    public Task Load(int? page = null, int? pageSize = null, string? q = null, string? sort = null, string? dir = null)
        => _mediator.Send(new AuditEventInitAction(page ?? Page, pageSize ?? PageSize, q ?? Query, sort ?? SortBy, dir ?? SortDir));

    public void Select(AuditEventDto? item)
    {
        Selected = item;
        Editing  = item is null
            ? new()
            : JsonSerializer.Deserialize<AuditEventDto>(
    ...
    public Task Handle(AuditEventInitEvent e, CancellationToken ct)
    {
        ReducePage(e.Page);
